...
var parsePaths = function (rootDir, paths, baseUrl, otherFiles, excludes) {
    ...
    var missingImports = (_a = []).concat.apply(_a, files.map(function (f) { return Object.keys(f.imports); })).filter(function (i) { return !found[i] && !excludes.some(function (exclude) { return i.indexOf(exclude, i.length - exclude.length) >= 0; }); })
...
exports.default = function (rootDir, paths, baseUrl, excludes) {
    return parsePaths(rootDir, paths, baseUrl, [], excludes || []);
};
